...
export default function EclipseCanvas({
  ...
  const stateRef = useRef({
    width: 0, height: 0, cx: 0, cy: 0, r: 0, dpr: 1,
  ...
  const resize = useCallback(() => {
    ...
    s.cx = s.width / 2;
    s.cy = s.height / 2;
    s.r = Math.min(s.width, s.height) * 0.43;
  }, []);
  ...
  // Draw a 4-pointed sparkle star at (x,y) with given size
  function drawSparkle(ctx: CanvasRenderingContext2D, x: number, y: number, sz: number) {
    ctx.beginPath();
    const outer = sz; const inner = sz * 0.28;
    for (let i = 0; i < 8; i++) {
  ...
  // Draw a simple 6-line asterisk at (x,y)
  function drawAsterisk(ctx: CanvasRenderingContext2D, x: number, y: number, sz: number) {
    for (let i = 0; i < 3; i++) {
      const a = (i / 3) * Math.PI;
      ctx.beginPath();
  ...
  function drawDiamond(ctx: CanvasRenderingContext2D, x: number, y: number, sz: number) {
    ctx.beginPath();
    ctx.moveTo(x, y - sz); ctx.lineTo(x + sz * 0.55, y);
    ctx.lineTo(x, y + sz); ctx.lineTo(x - sz * 0.55, y);
    ctx.closePath(); ctx.fill();
  }

  // Draw a circle + crosshair
  function drawCrosshair(ctx: CanvasRenderingContext2D, x: number, y: number, sz: number) {
  ...
  // Draw a small crescent moon
  function drawCrescent(ctx: CanvasRenderingContext2D, x: number, y: number, sz: number) {
    ctx.beginPath();
    ctx.arc(x, y, sz, -Math.PI * 0.75, Math.PI * 0.75);
    ctx.arc(x + sz * 0.45, y, sz * 0.8, Math.PI * 0.75, -Math.PI * 0.75, true);
    ctx.closePath(); ctx.fill();
  }

  // Draw a simple ring
  function drawRing(ctx: CanvasRenderingContext2D, x: number, y: number, sz: number) {
    ctx.beginPath(); ctx.arc(x, y, sz, 0, Math.PI * 2);
    ctx.arc(x, y, sz * 0.55, 0, Math.PI * 2, true);
    ctx.fill();
  }

  // 7 shape types corresponding to previous text glyphs
  type RuneDrawer = (ctx: CanvasRenderingContext2D, x: number, y: number, sz: number) => void;
  const RUNE_DRAWERS: RuneDrawer[] = [
    (ctx, x, y, sz) => drawSparkle(ctx, x, y, sz),          // ✦ sparkle
    (ctx, x, y, sz) => drawCrescent(ctx, x, y, sz),          // ☽ crescent
    (ctx, x, y, sz) => drawCrosshair(ctx, x, y, sz * 0.85),  // ⊕ crosshair
    (ctx, x, y, sz) => drawAsterisk(ctx, x, y, sz),          // ✶ asterisk
    (ctx, x, y, sz) => { ctx.beginPath(); ctx.arc(x, y, sz * 0.38, 0, Math.PI * 2); ctx.fill(); }, // ⋆ dot
    (ctx, x, y, sz) => drawSparkle(ctx, x, y, sz * 0.75),    // replaced ✴ with smaller sparkle
    (ctx, x, y, sz) => drawDiamond(ctx, x, y, sz),           // ⊗ diamond
  ];
  ...
  const drawCastingRunes = useCallback((ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, time: number, cr: number, cg: number, cb: number) => {
    ...
    const sz = Math.max(5, r * 0.055);
    const orbitR = r * 0.44;

    RUNE_DRAWERS.forEach((drawRune, i) => {
      const baseAngle = (i / RUNE_DRAWERS.length) * Math.PI * 2;
      ...
      ctx.strokeStyle = `rgba(255, 252, 245, ${pulse})`;
      ctx.lineWidth = Math.max(0.8, sz * 0.22);
      drawRune(ctx, gx, gy, sz);
      ctx.restore();
    });
  ...
  const drawTextInDisc = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    const targetFontSize = Math.max(17, Math.min(34, r * 0.155));
    const maxWidth = r * 1.62;

    let fontSize = targetFontSize;
    let lines: string[] = [];
    for (let attempt = 0; attempt < 4; attempt++) {
      const font = `700 italic ${fontSize}px 'Cormorant Garamond', 'Georgia', serif`;
      ctx.save();
      ctx.font = font;
      lines = wrapText(ctx, text, maxWidth);
      ctx.restore();
      if (lines.length <= 4) break;
      fontSize = Math.max(13, fontSize * 0.84);
    }
    ...
    lines.forEach((line, i) => {
      ...
      // subtle colour glow behind text
      ctx.save();
      ...
      // main text — bright white with stronger shadow for legibility
      ctx.save();
      ctx.globalAlpha = Math.min(1, alpha * 1.1);
      ctx.shadowBlur = 6;
      ctx.shadowColor = "rgba(0,0,0,0.9)";
      ctx.fillStyle = `rgba(255, 253, 248, ${Math.min(1, alpha * 1.1)})`;
      ctx.fillText(line, cx, lineY);
      ctx.restore();
    });
